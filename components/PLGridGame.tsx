...
export default function PLGridGame() {
  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        const rSaved   = localStorage.getItem(STORAGE_KEY(today, 'rarity'))
        const pSaved   = localStorage.getItem(STORAGE_KEY(today, 'popularity'))
        const seenSaved = localStorage.getItem(SEEN_KEY(today))
  ...
  function saveState(m: 'rarity' | 'popularity', guesses: Record<string, Guess>, score: number, done: boolean) {
    localStorage.setItem(STORAGE_KEY(today, m), JSON.stringify({ guesses, score, done }))
  }
  ...
  async function submitGuess(name: string) {
    ...
    if (mode === 'rarity') {
      ...
      saveState('rarity', next, nextScore, done)
    } else {
      ...
      saveState('popularity', next, nextScore, done)
    }
  }
  ...
  async function submitToLeaderboard() {
    if (!puzzle || !userName.trim() || submitted) return
    setSubmitting(true)
  ...
  function revealAnswers() {
    setAnswersSeen(true)
    localStorage.setItem(SEEN_KEY(today), 'true')
    setShowSkipConfirm(false)
    setActiveCell(null)
  }
    ...
    <div style={s.page}>
      ...
      <div style={{ maxWidth: 620, margin: '0 auto', padding: '16px 12px 60px' }}>
        ...
        <div style={{ textAlign: 'center', marginBottom: 14 }}>
          <div style={{ display: 'inline-block', background: '#1e2d4a', borderRadius: 20, padding: '3px 12px', fontSize: 11, color: '#8899bb', marginBottom: 8 }}>
            {puzzle.date}
          </div>
        ...
        {/* ── Switch mode prompt ── */}
        {currentDone && !bothDone && !answersSeen && (
        ...
        <div ref={gridRef} style={{ position: 'relative', marginBottom: 12 }}>
          <div style={{ display: 'grid', gridTemplateColumns: '117px 1fr 1fr 1fr', gap: 3 }}>
            ...
            {puzzle.rows.map((row, ri) => (
              <>
                ...
                {/* Cells */}
                {puzzle.cols.map((_, ci) => {
                  const key    = `${ri}_${ci}`
                  const guess  = guesses[key]
                  const count  = puzzle.answerCounts[key] ?? 0
          ...
          {answersSeen && overlayCell && overlayPos && (() => {
            ...
            const rowName  = puzzle.rows[ri]?.name ?? ''
            const colName  = puzzle.cols[ci]?.name ?? ''
        ...
        {activeCell && !currentDone && !answersSeen && (
          <div style={{ ...s.card, marginBottom: 12 }}>
            <div style={{ fontSize: 12, color: '#8899bb', marginBottom: 8 }}>
              {puzzle.rows[activeCell.r].name} × {puzzle.cols[activeCell.c].name}
            </div>
        ...
        {/* ── Reveal button if not seen yet and (at least one mode done OR answers not seen) ── */}
        {!answersSeen && !bothDone && (rarityDone || popDone) && (
