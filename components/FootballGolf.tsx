...
const NAT_LABELS: Record<string, string> = {
  ...
  COL:'Colombian',MLI:'Malian',TGO:'Togolese',TOG:'Togolese',NZL:'New Zealand',ZAF:'South African',RSA:'South African',
  CGO:'Congolese (Rep.)',SEY:'Seychellois',SCG:'Serbian/Montenegrin',GLP:'Guadeloupean',BER:'Bermudian',
...
const AUGUSTA_HAZARDS: Record<number, { hazards?: {start:number;end:number}[]; bunkers?: {start:number;end:number;side?:'left'|'right'}[] }> = {
...
const REAL_COURSE_HAZARDS: Record<number, { hazards?: {start:number;end:number}[]; bunkers?: {start:number;end:number;side?:'left'|'right'}[] }> = {
   2: {hazards:[{start:399,end:414}]},
   3: {bunkers:[{start:372,end:385}]},
   4: {bunkers:[{start:175,end:189}]},
   5: {bunkers:[{start:166,end:177}]},
   6: {hazards:[{start:249,end:354}]},
   7: {bunkers:[{start:83,end:93}]},
   8: {hazards:[{start:209,end:297}], bunkers:[{start:324,end:340}]},
  11: {bunkers:[{start:301,end:353}]},
  12: {bunkers:[{start:114,end:127},{start:167,end:187}]},
  13: {bunkers:[{start:138,end:159}]},
  14: {bunkers:[{start:173,end:227},{start:244,end:281}]},
  15: {bunkers:[{start:343,end:355}]},
  16: {bunkers:[{start:151,end:187},{start:345,end:368}]},
  17: {bunkers:[{start:151,end:173}]},
  18: {bunkers:[{start:390,end:444}]},
}
...
function buildCourseFromHoles(holeNums: number[], courseId: string): Hole[] {
  ...
  return holeNums.map(num => {
    const def = holeDefs.find(h => h.number === num) ?? holeDefs[0]
    const distance = def.yardages['Blue']
    const realHaz = hazardMap[def.number]
    return { number: def.number, par: def.par, distance, hazards: realHaz?.hazards ?? [], bunkers: realHaz?.bunkers ?? [], path: def.path }
  })
}
...
export default function FootballGolf(){
  ...
  type DailyRoundEntry = { player_name:string; total_strokes:number; score_to_par:number }
  const [dailyRoundMode, setDailyRoundMode]       = useState(false)
  const [dailyRoundDate, setDailyRoundDate]       = useState('')
  const [dailyRoundPlayerName, setDailyRoundPlayerName] = useState('')
  ...
  const [h2hHoleResults, setH2HHoleResults] = useState<('W'|'L'|'H')[]>([])
  ...
  async function startDailyRoundChallenge(playerName: string) {
    const today = new Date().toISOString().slice(0,10)
    const stored = localStorage.getItem(`golf_daily_round_${today}`)
    const resp = await fetch(`/api/golf-daily-round?date=${today}`)
    const data = await resp.json()
    setDailyRoundDate(today)
    setDailyRoundPlayerName(playerName)
    setDailyRoundLeaderboard(data.leaderboard ?? [])
    if (stored) {
      setDailyRoundResult(JSON.parse(stored))
      setPhase('daily-round-done')
      return
    }
    const builtHoles = buildCourseFromHoles(data.holeNumbers, data.course)
    setCourseMode('real')
    setSelectedCourse(data.course)
    setHoles(builtHoles)
    setHoleIdx(0)
    setScores([])
    setRemaining(builtHoles[0].distance)
    setStrokes(0)
    setShotResult(null)
    setPastPin(false)
    setDailyRoundResult(null)
    setDailyRoundMode(true)
    setQuestion(nextPickedCategory(builtHoles[0].distance))
    resetInputs()
    if (!localStorage.getItem('golf_rules_seen')) setShowRules(true)
    setPhase('playing')
  }
  ...
  function finishHole(finalStrokes:number){
    ...
    setTimeout(()=>{
      ...
      if(holeIdx+1>=holes.length){
        if(dailyRoundMode){
          const totalStrokes = newScores.reduce((s:number,n)=>s+(n??0), 0)
          const totalPar = holes.reduce((s,h)=>s+h.par, 0)
          const scoreToPar = totalStrokes - totalPar
          const res = { totalStrokes, scoreToPar }
          setDailyRoundResult(res)
          const today = new Date().toISOString().slice(0,10)
          localStorage.setItem(`golf_daily_round_${today}`, JSON.stringify(res))
          fetch('/api/golf-daily-round',{method:'POST',headers:{'Content-Type':'application/json'},
            body:JSON.stringify({date:dailyRoundDate,playerName:dailyRoundPlayerName,deviceId:getDeviceId(),totalStrokes,scoreToPar})
          }).then(r=>r.json()).then(d=>{ if(d.leaderboard) setDailyRoundLeaderboard(d.leaderboard) }).catch(()=>{})
          setDailyRoundMode(false)
          setPhase('daily-round-done')
        } else {
          setPhase('done')
        }
  ...
  function resolveH2HHole(myStrokes:number,oppStrokes:number){
    ...
    setH2HHoleResults(prev=>[...prev, delta===1?'W':delta===-1?'L':'H'])
  ...
  async function hostPlayAgain(){
    if (!metaReady) return
    setH2HRematchWaiting(true)
    const hs = courseMode === 'real'
      ? buildCourse(tee, numHoles, startHole, selectedCourse)
    ...
    await fetch('/api/golf-room', {
      method: 'POST', headers: {'Content-Type':'application/json'},
      body: JSON.stringify({action:'rematch', roomId: h2hRoomIdRef.current, holes: hs, teeCategories: teeCats}),
    })
    h2hRoomData.current = {holes: hs, teeCategories: teeCats}
    setH2HRematchWaiting(false)
    startH2HGame()
  }
      ...
      <div style={{...h2hScreenStyle,gap:24}}>
        ...
        <div style={{display:'flex',gap:12,width:'100%',maxWidth:340}}>
          <div style={{flex:1,background:'#111827',border:`2px solid ${iWon?'#22c55e':'#1e2d4a'}`,borderRadius:12,padding:'14px',textAlign:'center'}}>
            <div style={{fontSize:11,color:'rgba(255,255,255,0.4)',marginBottom:4}}>{myName}</div>
            <div style={{fontSize:28,fontWeight:900,color:iWon?'#22c55e':theyWon?'#ef4444':'#94a3b8'}}>{matchScore>0?`+${matchScore}`:matchScore<0?matchScore:'-'}</div>
          </div>
          <div style={{flex:1,background:'#111827',border:`2px solid ${theyWon?'#22c55e':'#1e2d4a'}`,borderRadius:12,padding:'14px',textAlign:'center'}}>
            <div style={{fontSize:11,color:'rgba(255,255,255,0.4)',marginBottom:4}}>{oppName}</div>
  ...
  if(phase==='daily-round-setup') return <><NavBar /><DailyRoundSetupScreen onPlay={startDailyRoundChallenge} onBack={()=>setPhase('setup')} /></>
  if(phase==='daily-round-done')  return <><NavBar /><DailyRoundDoneScreen result={dailyRoundResult} leaderboard={dailyRoundLeaderboard} playerName={dailyRoundPlayerName} onBack={()=>{setDailyRoundResult(null);setDailyRoundLeaderboard([]);setPhase('setup')}} /></>
    ...
    <div style={{minHeight:'100dvh',background:'#0a0f1e',fontFamily:"'DM Sans',-apple-system,sans-serif"}}>
      ...
      <div style={{maxWidth:560,margin:'0 auto',width:'100%',padding:'0 20px'}}>
        ...
        <div style={{display:'flex',alignItems:'stretch',height:'calc(50dvh + 172px)',position:'relative'}}>
          ...
          {/* Left panel */}
          <div style={{flex:13,padding:'12px 8px 20px',display:'flex',flexDirection:'column',gap:10,minWidth:0,position:'relative',zIndex:1}}>
            <Scorecard holes={holes} scores={scores} currentIdx={holeIdx} course={courseMode === 'real' ? selectedCourse : 'random'} h2hHoleResults={h2hStep==='playing'?h2hHoleResults:undefined} />
            {/* Hole info */}
              ...
              <div style={{border:'1px solid rgba(255,255,255,0.07)',borderRadius:12,padding:'8px 8px 8px',display:'flex',flexDirection:'column',gap:6}}>
                ...
                <div style={{borderTop:'1px solid rgba(255,255,255,0.07)',paddingTop:6,marginTop:2}}>
                  <button
                    ...
                    style={{
                      width:'100%',background:confirmedPlayers.every(p=>!p)||isAnimating?'transparent':'#dc2626',
          ...
          {/* Right panel — overall score + hole view */}
...
{!imageUrl && !hole.isIsland && hole.bunkers.map((b,i)=>{
        ...
        {displayBallPos <= hole.distance && hole.bunkers.map((b,i)=>{
          ...
          const fs = labelFs * 0.85
          const pw = imageUrl ? 40 : 20; const ph = imageUrl ? 10 : 5
          const onRight = b.side ? b.side === 'right' : mid.x <= 50
          const px = onRight ? mid.x + 6 : mid.x - pw - 6; const py = mid.y - ph/2
          return (
            <g key={i}>
...
function ScoreCell({score,par}:{score:number;par:number}){
  ...
  const outer={display:'flex',alignItems:'center',justifyContent:'center',width:18,height:18}
  const num={display:'flex',alignItems:'center',justifyContent:'center',fontSize:10,fontWeight:900}
  if(diff<=-2) return(
    <div style={{...outer,borderRadius:'50%',border:'2px solid #22c55e'}}>
      <div style={{...num,width:11,height:11,borderRadius:'50%',border:'1.5px solid #22c55e',color:'white'}}>{score}</div>
    </div>
  )
  if(diff===-1) return(
    <div style={{...outer,...num,borderRadius:'50%',background:'#22c55e',color:'#0a0f1e'}}>{score}</div>
  )
  if(diff===0) return(
    <div style={{...outer,...num,color:'white'}}>{score}</div>
  )
  if(diff===1) return(
    <div style={{...outer,...num,background:'#ef4444',borderRadius:3,color:'white'}}>{score}</div>
  )
  return(
    <div style={{...outer,borderRadius:3,border:'2px solid #ef4444'}}>
      <div style={{...num,width:11,height:11,borderRadius:2,background:'#ef4444',color:'white'}}>{score}</div>
    </div>
  )
}
...
function Scorecard({holes,scores,currentIdx,course,h2hHoleResults}:{
  holes:Hole[];scores:(number|null)[];currentIdx:number;course?:string;h2hHoleResults?:('W'|'L'|'H')[]
}){
  const ROW_H = 20
  const COL_W = 28
  const LABEL_W = 46
  ...
  useEffect(() => {
    ...
    const totalW = holes.length * COL_W
    const maxScroll = Math.max(0, totalW - availW)
    const desiredScroll = currentIdx * COL_W - Math.floor((availW - COL_W) / 2)
    scroll.scrollLeft = Math.max(0, Math.min(desiredScroll, maxScroll))
  }, [currentIdx, holes.length])
  ...
  return(
    <div style={{padding:'6px 0',display:'flex',alignItems:'center',gap:8}}>
      ...
      <div ref={containerRef} style={{flex:1,display:'flex',minWidth:0}}>
        {/* Label column */}
        ...
        {/* Scrolling hole columns */}
        <div ref={scrollRef} style={{flex:1,overflow:'hidden',display:'flex',minWidth:0}}>
          {holes.map((h,i)=>{
            ...
            return(
              <div key={i} style={{width:COL_W,flexShrink:0,borderLeft:'1px solid rgba(255,255,255,0.07)'}}>
                ...
                <div style={{height:ROW_H,display:'flex',alignItems:'center',justifyContent:'center'}}>
                  {h2hHoleResults && i < h2hHoleResults.length ? (
                    h2hHoleResults[i]==='W'
                      ? <div style={{display:'flex',alignItems:'center',justifyContent:'center',width:18,height:18,borderRadius:'50%',background:'#22c55e',fontSize:9,fontWeight:900,color:'#0a0f1e'}}>W</div>
                      : h2hHoleResults[i]==='L'
                        ? <div style={{display:'flex',alignItems:'center',justifyContent:'center',width:18,height:18,borderRadius:3,background:'#ef4444',fontSize:9,fontWeight:900,color:'white'}}>L</div>
                        : <div style={{fontSize:10,fontWeight:700,color:'rgba(255,255,255,0.7)'}}>H</div>
                  ) : scores[i]==null
...
const HOLE_POSITIONS: Record<number, {teeFrac:[number,number]; greenFrac:[number,number]; waypointFracs?:[number,number][]}> = {
   1: {teeFrac:[0.808,0.919], greenFrac:[0.529,0.107], waypointFracs:[[0.24,0.581],[0.249,0.463]]},
   2: {teeFrac:[0.69,0.92],   greenFrac:[0.432,0.088], waypointFracs:[[0.548,0.446],[0.497,0.273]]},
   3: {teeFrac:[0.175,0.929], greenFrac:[0.291,0.097], waypointFracs:[[0.856,0.559]]},
   4: {teeFrac:[0.608,0.937], greenFrac:[0.573,0.102]},
   5: {teeFrac:[0.331,0.922], greenFrac:[0.502,0.168]},
   6: {teeFrac:[0.631,0.912], greenFrac:[0.481,0.083], waypointFracs:[[0.663,0.319]]},
   7: {teeFrac:[0.513,0.804], greenFrac:[0.567,0.261]},
   8: {teeFrac:[0.742,0.834], greenFrac:[0.565,0.1],   waypointFracs:[[0.211,0.325]]},
   9: {teeFrac:[0.562,0.92],  greenFrac:[0.553,0.11],  waypointFracs:[[0.345,0.547]]},
  10: {teeFrac:[0.568,0.932], greenFrac:[0.63,0.1],    waypointFracs:[[0.427,0.408],[0.441,0.332]]},
  11: {teeFrac:[0.458,0.922], greenFrac:[0.595,0.1],   waypointFracs:[[0.458,0.327]]},
  12: {teeFrac:[0.609,0.89],  greenFrac:[0.527,0.134], waypointFracs:[[0.465,0.389]]},
  13: {teeFrac:[0.556,0.953], greenFrac:[0.444,0.114], waypointFracs:[[0.367,0.384],[0.338,0.329]]},
  14: {teeFrac:[0.662,0.912], greenFrac:[0.708,0.096], waypointFracs:[[0.127,0.46]]},
  15: {teeFrac:[0.443,0.901], greenFrac:[0.517,0.099], waypointFracs:[[0.68,0.462],[0.749,0.436]]},
  16: {teeFrac:[0.777,0.926], greenFrac:[0.652,0.128], waypointFracs:[[0.415,0.486]]},
  17: {teeFrac:[0.589,0.89],  greenFrac:[0.424,0.226]},
  18: {teeFrac:[0.297,0.904], greenFrac:[0.382,0.089], waypointFracs:[[0.862,0.433]]},
}
...
function DailySetupScreen({onPlay,onBack}:{onPlay:(name:string)=>void;onBack:()=>void}){
  ...
  return(
    <div style={{minHeight:'calc(100dvh - 56px)',background:'#0a0f1e',display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'flex-start',gap:24,fontFamily:"'DM Sans',sans-serif",padding:'16px 24px'}}>
      <style>{`@import url('https://fonts.googleapis.com/css2?family=DM+Sans:wght@400;700;800;900&display=swap');*{box-sizing:border-box;}`}</style>
...
function DailyRoundSetupScreen({onPlay,onBack}:{onPlay:(name:string)=>void;onBack:()=>void}){
  const [name,setName] = useState(getSavedUsername)
  const [loading,setLoading] = useState(false)
  const todayKey = new Date().toISOString().slice(0,10)
  const alreadyPlayed = typeof window!=='undefined' && !!localStorage.getItem(`golf_daily_round_${todayKey}`)
  const todayLabel = new Date().toLocaleDateString('en-GB',{weekday:'long',day:'numeric',month:'long'})
  function submit(){
    const n=name.trim()
    if(!n) return
  ...
  return(
    <div style={{minHeight:'calc(100dvh - 56px)',background:'#0a0f1e',display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'flex-start',gap:24,fontFamily:"'DM Sans',sans-serif",padding:'16px 24px'}}>
      ...
      <div style={{textAlign:'center'}}>
        ...
        <div style={{fontSize:26,fontWeight:900,color:'white',letterSpacing:'-0.5px'}}>3-Hole Round</div>
        <div style={{fontSize:13,color:'rgba(255,255,255,0.4)',marginTop:6}}>{todayLabel}</div>
      </div>
      <div style={{background:'#111827',border:'1px solid #1e2d4a',borderRadius:14,padding:'16px 20px',width:'100%',maxWidth:320,textAlign:'center'}}>
        <div style={{fontSize:12,color:'rgba(255,255,255,0.4)',marginBottom:6,fontWeight:700,textTransform:'uppercase',letterSpacing:'0.06em'}}>Today's Challenge</div>
        <div style={{fontSize:15,fontWeight:800,color:'white'}}>3 Seeded Holes</div>
        <div style={{fontSize:12,color:'rgba(255,255,255,0.4)',marginTop:4}}>Pebble Beach or Augusta · lowest score wins</div>
      </div>
      {alreadyPlayed&&(
        <div style={{background:'rgba(34,197,94,0.1)',border:'1px solid rgba(34,197,94,0.3)',borderRadius:12,padding:'14px 20px',width:'100%',maxWidth:320,textAlign:'center'}}>
          <div style={{fontSize:14,fontWeight:800,color:'#22c55e'}}>✓ Already played today</div>
      ...
      <div style={{width:'100%',maxWidth:320}}>
        <div style={{fontSize:11,fontWeight:800,color:'rgba(255,255,255,0.35)',textTransform:'uppercase',letterSpacing:'0.08em',marginBottom:8}}>Your name</div>
        <input value={name} onChange={e=>setName(e.target.value)} onKeyDown={e=>e.key==='Enter'&&submit()} placeholder="Enter your name…"
          style={{width:'100%',background:'#111827',border:'1px solid #1e2d4a',borderRadius:10,padding:'13px 14px',fontSize:16,color:'white',fontFamily:'inherit',outline:'none'}}/>
      </div>
      <button onClick={submit} disabled={!name.trim()||loading}
        style={{background:name.trim()&&!loading?'#16a34a':'#1e2d4a',color:'white',border:'none',borderRadius:12,padding:'14px 52px',fontSize:16,fontWeight:900,cursor:name.trim()&&!loading?'pointer':'default',fontFamily:'inherit',transition:'background 0.15s'}}>
        {loading?'Loading…':alreadyPlayed?'View Leaderboard →':'Tee Off →'}
      </button>
    </div>
  )
...
function DailyRoundDoneScreen({result,leaderboard,playerName,onBack}:{
  result:{totalStrokes:number;scoreToPar:number}|null
  ...
  return(
    <div style={{minHeight:'calc(100dvh - 56px)',background:'#0a0f1e',fontFamily:"'DM Sans',sans-serif",padding:'16px 24px'}}>
      ...
      <div style={{textAlign:'center',marginBottom:20}}>
        <div style={{fontSize:24,fontWeight:900,color:'white'}}>🏌️ 3-Hole Round</div>
        <div style={{fontSize:12,color:'rgba(255,255,255,0.35)',marginTop:4}}>{today}</div>
      </div>

      {result&&(
        <div style={{background:'#111827',border:`2px solid ${scoreColor(result.scoreToPar)}`,borderRadius:14,padding:'18px 20px',marginBottom:20,textAlign:'center'}}>
          <div style={{fontSize:13,color:'rgba(255,255,255,0.4)',marginBottom:4,fontWeight:700,textTransform:'uppercase',letterSpacing:'0.06em'}}>Your score</div>
          <div style={{fontSize:40,fontWeight:900,color:scoreColor(result.scoreToPar)}}>{scoreLabel(result.scoreToPar)}</div>
          <div style={{fontSize:13,color:'rgba(255,255,255,0.5)',marginTop:4}}>{result.totalStrokes} strokes total</div>
          {myRank&&<div style={{marginTop:8,fontSize:13,fontWeight:800,color:'#22c55e'}}>#{myRank} on today's leaderboard</div>}
        </div>
      )}
      {!result&&(
        <div style={{background:'#111827',border:'1px solid #1e2d4a',borderRadius:14,padding:'14px 20px',marginBottom:20,textAlign:'center'}}>
          ...
          {leaderboard.find(e=>e.player_name===playerName) && (
            <div style={{fontSize:22,fontWeight:900,marginTop:6,color:scoreColor(leaderboard.find(e=>e.player_name===playerName)!.score_to_par)}}>
              {scoreLabel(leaderboard.find(e=>e.player_name===playerName)!.score_to_par)}
            </div>
          )}
        </div>
      )}
        ...
        {(!clubView||!clubLoading)&&board.map((e,i)=>{
          ...
          return(
            <div key={i} style={{background:isMe?'rgba(22,163,74,0.1)':'#111827',border:`1px solid ${isMe?'rgba(22,163,74,0.4)':'#1e2d4a'}`,borderRadius:10,padding:'11px 14px',display:'flex',alignItems:'center',gap:12}}>
              <div style={{width:28,fontSize:14,fontWeight:900,color:i===0?'#fbbf24':i===1?'#9ca3af':i===2?'#cd7c2f':'rgba(255,255,255,0.3)',textAlign:'center'}}>#{i+1}</div>
              <div style={{flex:1,fontSize:14,fontWeight:isMe?800:600,color:isMe?'white':'rgba(255,255,255,0.7)'}}>{e.player_name}</div>
              <div style={{fontSize:14,fontWeight:900,color:scoreColor(e.score_to_par)}}>{scoreLabel(e.score_to_par)}</div>
              <div style={{fontSize:11,color:'rgba(255,255,255,0.3)',minWidth:32,textAlign:'right'}}>{e.total_strokes} str</div>
            </div>
          )
        })}
      </div>
